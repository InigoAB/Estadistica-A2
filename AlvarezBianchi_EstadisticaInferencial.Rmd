...
Obtengo que la media de ventas, con un nivel de confianza del 95%, se encuentra en el intervalo: [`r icm(CCS$Sales)`]
...
$H_{0}: \mu_{1}=\mu_{2}$ o $H_{0}: \mu_{1}-\mu_{2}\leq0$
...
CH_UD_m2pi_vd <- function(pobl1, pobl2, NC=0.95) {
  ...
  data.frame(L=tcritL, U="INF", tobs, pvalue)
}
...
El valor p es muy pequeño por lo que rechazo la hipótesis nulas de que las ventas en Estados Unidos y fuera son iguales. Puedo concluir que las ventas en Estados Unidos son más altas con una confianza del 95%. (Parece que he debido de cometer algún error que no consigo detectar porque el valor observado "tobs" cae en la zona de aceptación.)

## 3.4 Conclusión

El valor p es mucho más pequeño que α, por lo que hay que rechazar la hipótesis nula.
...
CH_UD_m2pi_vi <- function(pobl1, pobl2, NC=0.95) {
  ...
  data.frame(L=tcritL, U=tcritU, tobs, pvalue)
}
...
Apliaré un test sobre la proporción para una muestra grande ya que contamos con una muestra grande, de n = `r nrow(CCS)`.
...
El valor crítico ha caído en 1.64 lo que implica que la zona de aceptación es (-∞, 1,64] por lo que con un valor observado 7.04, que cae fuera, rechzamos la hipótesis nula. El valor p es muy pequeño, inferior a la significación fijada del 0.05 así que esto también nos lleva a rechazar la hipótesis nula.
...
Aplicaré un test sobre la proporción de dos muestras.
